...
{
    if (std::lock_guard lock(Impl::getUnsharedGlObjectsMutex());
        auto            unsharedGlObjects = Impl::getWeakUnsharedGlObjects().lock())
        unsharedGlObjects->emplace_back(Impl::UnsharedGlObject{GlContextImpl::CurrentContext::get().id, object});
}
...
std::unique_ptr<GlContext> GlContext::create()
{
    ...
    std::lock_guard lock(sharedContext->mutex);
...
void GlContext::cleanupUnsharedResources()
{
    ...
    {
        std::lock_guard lock(Impl::getUnsharedGlObjectsMutex());
